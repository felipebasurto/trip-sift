# Search depth: native trips, stop table, Google Hotels, MCP

## Context

viajante already owns sweep, detail, evidence JSON, baggage ranking, Booking hotels, and destination triage. The gaps that change a search are still open: real round-trip and multi-city packages, a shopping stop table that matches the query, a no-Chromium hotel shortlist, and an MCP server agents can discover.
...
**A. Extend owned types and encoders.** Chosen. The tree already owns search `tfs` and shopping JSON.

**B. Vendor an external shopping client.** Rejected. Extra pydantic, a generated airport enum, a booking-page `tfs` that is a different token, and a second HTTP client.

**C. Feature-table race (watches, hacks, ticketing, TypeScript).** Rejected. Those delete the no-keys promise or the honest-totals stance.
...
- Keep a row in `docs/plans/01-search-depth/decisions.tsv` when a phase picks a fork the overview did not name.
...
- Do not copy a third-party filter list (slice 8).
- Skip Booking HTTP (slice 6). Google Hotels is the hotel sweep (slice 5). Default source stays Booking.
- MCP is five named tools (slice 7), not a generic travel-agent router.
