...
- `tests/test_google_flights.py`. Offline list fixtures. Do not import fli. Cross-check slot numbers by reading fli's test file when writing fixtures.
...
Gate. Do not merge hypothesized multi-city TFS field 19 until a captured Google search URL confirms it. fli's booking `tfs` is a different token. Do not use it.
